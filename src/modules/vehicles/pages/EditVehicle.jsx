...
export const EditVehicle = () => {
  ...
  useEffect(() => {
    ...
    if (vehicle) {
      setFormData({
        ...
        phVehicleLicence: vehicle.ph_vehicle_licence_no || '',
        licenceExpiry: vehicle.licence_expiry || '',
        providerName: vehicle.insurence_provider || '',
      ...
      setDriverSearch(vehicle.driver || '');
    }
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await FunctionsAPI.editVehicle({ type: "edit", id, fields: formData });
      if (res.success) {
    ...
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto pb-12 px-4 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Edit Vehicle {id}</h2>
            <p className="text-slate-500 font-medium mt-1">Modify vehicle specifications and compliance records for {formData.make} {formData.model}.</p>
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="text" className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 font-semibold outline-none cursor-not-allowed" value={id} disabled />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <div className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border border-slate-300" style={{ backgroundColor: formData.colour?.toLowerCase() }}></div>
                  <input 
                    type="text" 
                    name="colour"
      ...
      <div className="m-8 flex items-center justify-end gap-4 px-4">
        ...
        <button type="submit" className="bg-primary-600 text-white px-10 py-3.5 rounded-xl font-bold text-sm hover:bg-primary-700 active:scale-[0.98] transition-all shadow-lg shadow-primary-600/20 flex items-center gap-2.5">
