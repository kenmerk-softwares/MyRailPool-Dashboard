...
export const EditRoute = () => {
  const { id } = useParams();
  const route = routesData.find(r => r.id.replace('#', '') === id) || routesData[0];
    ...
    <div className="max-w-6xl mx-auto pb-12 px-4 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Edit Route {route.id}</h2>
            <p className="text-slate-500 font-medium mt-1">Modify operational parameters and asset allocation for this corridor.</p>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    defaultValue={route.estPrice}
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Outbound Window (Out)</label>
                <div className="relative">
                  <Clock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input
                    type="text"
                    className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 bg-white text-slate-800 font-medium focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 outline-none transition-all"
                    defaultValue={route.timings}
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Inbound Window (Return)</label>
                <div className="relative">
      ...
      <div className="mt-10 flex items-center justify-end gap-4 px-4">
        ...
        <button className="bg-primary-600 text-white px-10 py-3.5 rounded-xl font-bold text-sm hover:bg-primary-700 active:scale-[0.98] transition-all shadow-lg shadow-primary-600/20 flex items-center gap-2.5">
          <Save className="w-4.5 h-4.5" /> Update Route
        </button>
      </div>
    </div>
  );
