        ...
        <table className="w-full text-left border-collapse min-w-[700px]">
          <thead>
            <tr className="bg-slate-50/50">
              <th className="px-4 md:px-6 py-4 text-xs md:text-sm font-semibold text-slate-500 border-b border-slate-100">Model & Plate</th>
            ...
            {vehiclesData.map((vehicle, idx) => (
              <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                <td className="px-4 md:px-6 py-4">
                  ...
                  <div className="text-[10px] md:text-xs text-slate-500 mt-0.5">{vehicle.plate}</div>
                </td>
                ...
                <td className="px-4 md:px-6 py-4 text-xs md:text-sm">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${vehicle.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'}`}>
                    {vehicle.status}
                  </span>
                </td>
